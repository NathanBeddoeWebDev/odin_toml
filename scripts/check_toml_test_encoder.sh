...
odin build cmd/toml_test_encoder -file \
  -collection:external="$repo_root/external" \
  -out:build/tools/toml-test-encoder \
  -vet -vet-style -warnings-as-errors

report=build/reports/toml-test-encoder.json
build/tools/toml-test test \
  -toml=1.1.0 \
  -decoder=./build/tools/toml-test-decoder \
  -encoder=./build/tools/toml-test-encoder \
  -timeout=5s \
  -parallel=4 \
  -color=never \
  -json > "$report"

preserved=tests/corpus/toml-test-encoder-report.json
evidence=build/reports/toml-test-encoder-evidence.json
python3 - \
...
expected_decoder = ["./build/tools/toml-test-decoder"]
expected_encoder = ["./build/tools/toml-test-encoder"]
if report.get("decoder") != expected_decoder:
    failures.append(f"decoder: expected {expected_decoder!r}, got {report.get('decoder')!r}")
if report.get("encoder") != expected_encoder:
    failures.append(f"encoder: expected {expected_encoder!r}, got {report.get('encoder')!r}")
...
for required in (
    ...
    if required not in flags:
        failures.append(f"runner report does not contain literal {required}")
...
if failures:
    raise SystemExit("official encoder gate failed:\n- " + "\n- ".join(failures))
...
print(
    "official TOML encoder gate passed: "
    f"pin={tag}@{revision} toml={report['toml']} "
    f"decoder={report['passed_valid']} encoder={report['passed_encoder']} "
    f"invalid={report['passed_invalid']} skips=0 platform={runtime_platform}"
)
PY

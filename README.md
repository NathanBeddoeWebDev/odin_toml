...
Early public-seam fuzz targets live in `tests/semantic_fuzz` and beside both conformance adapters. Every ordinary run reports its `ODIN_TEST_RANDOM_SEED`, covers 4,096 arbitrary byte cases and 2,048 valid-UTF-8 seed mutations, applies semantic and canonical round-trip checks to accepted TOML, validates malformed owners without unsafe salvage destruction, and varies writer counts/errors while checking canonical prefixes and no retries. Adapter targets additionally exercise 4,096 arbitrary inputs with observed allocation cleanup. Minimized findings are retained as deterministic literal fixtures in the nearest public-seam suite.

`tests/semantic_fuzz` also builds as a one-artifact replay target with the selectors `strict-parse`, `valid-utf8`, `parse-unparse`, `semantic-lifecycle`, and `writer-validation`. The test encoder's `--fuzz-target` mode provides the malformed tagged-JSON target. These entrypoints read one artifact from standard input, assert public-seam invariants, and are suitable for the later sanitizer-backed coverage-guided campaign without changing the replay contract.

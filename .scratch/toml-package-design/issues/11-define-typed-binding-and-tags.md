...
Unmarshal requires a matched destination map slot to be nil/zero, initializes it with the call allocator, and binds values recursively in semantic table insertion order. Destination key strings are independently owned. Each key/value pair is a commit unit: construct an uninstalled pair under package cleanup responsibility, install it only when complete, and then transfer it to the map. On failure, earlier entries and the map remain destination-owned; an uncommitted pair is cleaned by the package. Typed unmarshal never clears or merges into an existing map.
